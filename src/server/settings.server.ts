// Per-user settings: time zone and week start (docs/architecture.md, "Time zones"). They
...
const columns = { timeZone: userSettings.timeZone, weekStart: userSettings.weekStart }

// Returns the user's settings, creating them on the first call with the browser's time
// zone and a Monday week start. Later calls ignore input.timeZone.
export async function getSettings(db: Database, userId: string, input: GetSettingsInput) {
  ...
  await db
    .insert(userSettings)
    .values({ userId, timeZone: input.timeZone })
    .onConflictDoNothing()
...
export async function updateSettings(db: Database, userId: string, input: UpdateSettingsInput) {
  const [updated] = await db
    .update(userSettings)
    .set({ timeZone: input.timeZone, weekStart: input.weekStart })
    .where(eq(userSettings.userId, userId))
    .returning(columns)
  if (!updated) throw new AppError('NOT_FOUND', 'Load the settings first.')
  return updated
}
