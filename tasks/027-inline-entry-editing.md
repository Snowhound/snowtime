...
- [ ] A prototype control switches entry editing between Dialog (today's behavior) and
      Inline, in every layout, keeping the fixture and layout across switches
- [ ] Description: the row's text is an input on focus or click; Enter or blur saves,
      Escape restores the saved value
- [ ] Project: a dropdown in the row with "No project", the active projects with their
...
- [ ] Start and end: time inputs in the row. An end at or before the start means the
      next day, as in the dialog, and the duration updates as the user types
- [ ] Duration: the prototype decides whether it is editable (changing the end) or stays
      read-only, and records the choice
- [ ] Date: the prototype decides how an entry moves to another day (a date input in the
      row, a small popover, or the dialog kept for that), and the row moves to its new
      day group after saving
- [ ] Each committed field saves on its own, modeled as `updateEntry` with only that
      field, optimistic, with a simulated failure that rolls the field back and shows an
      error on the row
- [ ] Validation matches the dialog (times required, no end in the future), shown on the
      field without saving; the invalid value stays until fixed or Escape
- [ ] Keyboard: Tab moves through a row's fields in order, focus is visible on each, and
      the continue and delete actions stay reachable
- [ ] 390 px: the prototype decides whether rows edit inline or open the dialog, since
      five controls don't fit one line
- [ ] Manual entry ("Add entry"): the prototype decides whether it adds an empty row
      inline or keeps opening the dialog
- [ ] Fixtures: running, idle, long content, empty, plus an archived project and an
      entry crossing midnight
- [ ] Checks from `prototypes/README.md` pass, and the timer reference entry in that
      README describes inline editing and its decisions
- [ ] A follow-up task ports the chosen design, and task 023 subtask 05 (Table layout)
