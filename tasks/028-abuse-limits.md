...
- [ ] Better Auth's rate limiter works across Vercel instances. Better Auth turns it on in
      production but keeps counts in memory by default, and each function instance
      has its own memory. Pick `secondaryStorage` (for example Upstash Redis on its free
      tier) or `storage: 'database'`, which costs a Turso write per counted request.
- [ ] `customRules` in Better Auth's `rateLimit` set stricter limits for
...
- [x] `organizationLimit` caps how many organizations one user can create (unlimited by
      default).
- [ ] Server functions that write (entries, projects, teams) have a per-user limit.
...
- [ ] The limits (done) and the storage choice are recorded in `docs/architecture.md`.
