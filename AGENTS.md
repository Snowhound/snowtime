...
- Icon components end in `Icon`, so JSX shows what they are. Import a Lucide icon as its
  ...
  `GoogleIcon`.
- Import from another folder of `src/` through the `~/` alias (`~/lib/utils`), not with
  `../`; files in the same folder import with `./`. oxlint's
  `import/no-relative-parent-imports` checks this.
